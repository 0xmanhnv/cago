...
@frappe.whitelist()
def search_products(query=None, recommended_only=0, sort=None, start=0, limit=24):
	"""Owner product search -> list of owner DTOs (paginated for the product list's load-more).
	`sort`: newest | name_asc | name_desc | price_asc | price_desc (None = default browse/search order)."""
	ensure_cap("products")
	from frappe.utils import cint
